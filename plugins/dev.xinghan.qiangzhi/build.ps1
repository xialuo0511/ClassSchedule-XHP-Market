...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $PSScriptRoot 'dist\qiangzhi-import.xhp'
}
...
if ($manifest.protocolVersion -ne 3) {
    throw 'Example plugin.json must use package protocolVersion 3'
}
if (@($manifest.capabilities) -notcontains 'course_import') {
    throw 'Example plugin.json must declare course_import'
}
...
$temporaryZip = Join-Path $outputDirectory 'qiangzhi-import.tmp.zip'
